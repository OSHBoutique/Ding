...
if (spollers.length > 0) {
	function spollerCLick(e) {
		...
		if (spollersGo) {
			...
			// Снимаем _default-open, чтобы переключение работало
			if (spoller.classList.contains('_default-open')) {
				spoller.classList.remove('_default-open');
			}

			// Закрываем другие спойлеры в режиме _one
			if (spoller.closest('._spollers').classList.contains('_one')) {
				let currentSpollers = spoller.closest('._spollers').querySelectorAll('._spoller');
				currentSpollers.forEach((el) => {
					if (el !== spoller) {
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  function updateSaveButtonState() {
    const isMainAddressFilled = checkInputs(mainAddressInputs);
    const isDeliveryAddressFilled =
      checkbox.checked || checkInputs(deliveryAddressInputs);
    saveButton.disabled = !(isMainAddressFilled && isDeliveryAddressFilled);
    checkoutButton.disabled = !(isMainAddressFilled && isDeliveryAddressFilled);
  }
  ...
  function handleSave() {
    ...
    const billingAddress = collectFormData(mainAddressInputs);
    const deliveryAddressData = checkbox.checked
      ? billingAddress
      : collectFormData(deliveryAddressInputs);
...
document.addEventListener("DOMContentLoaded", () => {
  const addToCartButtons = document.querySelectorAll(".checkout__add");
    ...
    if (cart.length === 0) {
      ...
      cart.forEach((item) => {
        const totalItemPrice =
          (item.price + item.price * item.vat) * item.quantity;
  ...
  function updateTotals() {
    ...
    subtotalElementPayment.textContent = `${subtotal.toFixed(2)} €`;
    vatElementPayment.textContent = `${totalVat.toFixed(2)} €`;
    totalElementPayment.textContent = `${total.toFixed(2)} €`;
  }
